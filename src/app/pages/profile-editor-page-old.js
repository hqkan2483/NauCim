let profileData = {
      ...
      function syncLeftTreeWithRightItem(rightItemKey) {
        ...
        if (matchingLeftKey) {
          ...
          if (modelObjectPanel) {
            ...
            let html = `
              <div class="tabs">
                <div class="tab active" onclick="switchEditorTab('description')">Описание</div>
                ${isElement ? `<div class="tab" onclick="switchEditorTab('attributes')">Атрибуты</div>` : ''}
                ${isElement ? `<div class="tab" onclick="switchEditorTab('links')">Связи</div>` : ''}
              </div>

              <div class="tab-content active" data-tab="description">
                <p style="color: #888; font-style: italic; padding: 10px 0;">
                  Соответствующий объект не найден в доступных моделях.
                </p>
              </div>
            `;
            ...
            if (isElement) {
              ...
              <div class="tab-content" data-tab="attributes">
                <h4 style="margin-top: 0;">Атрибуты</h4>
              ...
              <div class="tab-content" data-tab="links">
                <h4 style="margin-top: 0;">Связи</h4>
                <p style="color: #888; font-style: italic;">
                  Соответствующий объект не найден в доступных моделях.
                </p>
              </div>
      ...
      function syncProfileTreeWithLeftItem(leftItemKey) {
        ...
        if (matchingProfileKey) {
          ...
          if (profileObjectPanel) {
            ...
            let html = `
              <div class="tabs">
                <div class="tab active" onclick="switchProfileTab('description')">Описание</div>
                ${isElement ? `<div class="tab" onclick="switchProfileTab('attributes')">Атрибуты</div>` : ''}
                ${isElement ? `<div class="tab" onclick="switchProfileTab('links')">Связи</div>` : ''}
              </div>

              <div class="tab-content active" data-tab="description">
                <p style="color: #888; font-style: italic; padding: 10px 0;">
                  Данный объект не найден в текущем профиле.
                  Вы можете добавить его, используя кнопку "→ Перенести в профиль".
                </p>
              </div>
            `;
      ...
      function transferToProfile() {
        ...
        if (transferredCount > 0) {
          ...
          alert(`Успешно перенесено элементов: ${transferredCount}`);
        } else {
          alert("Не удалось перенести элементы");
        }
      }
      ...
      function saveProfileToMemoryStore() {
        ...
        if (profile) {
          ...
          console.log("✅ Profile saved to MemoryStore:", profile.name);
        }
      }
        ...
        if (removedCount > 0) {
          ...
          if (profileContainer) {
            profileContainer.innerHTML =
              '<div class="text-muted" style="padding: 10px 0;">Здесь будет отображаться информация из выбранного профиля.</div>';
          }

          alert(`Успешно удалено элементов: ${removedCount}`);
        } else {
          alert("Не удалось удалить элементы");
        }
      }
      ...
      function setEditorMode() {
        ...
        if (contentArea.classList.contains("standart-mode")) {
          ...
          toggleBtn.textContent = "📋 Стандартный режим";
          ...
          toggleBtn.textContent = "📐 Режим диаграммы";
      ...
      function initializeDiagramEditor() {
        ...
        container.innerHTML = `
          <div class="diagram-toolbar">
            <button class="diagram-tool-btn" onclick="setLinkMode('generalization')" id="gen-btn" title="Создать связь обобщения">
              🔺 Обобщение
            </button>
            <button class="diagram-tool-btn" onclick="setLinkMode('association')" id="assoc-btn" title="Создать связь ассоциации">
              ➡️ Ассоциация
            </button>
            <button class="diagram-tool-btn" onclick="clearLinkMode()" title="Отменить режим связи">
              ❌ Отмена
            </button>
            <button class="diagram-tool-btn" onclick="clearDiagram()" title="Очистить диаграмму">
              🗑️ Очистить
            </button>
          </div>
        ...
        diagramPaper.on("element:pointerclick", function (elementView, evt) {
          if (currentLinkMode) {
            ...
            if (!startLinkFrom) {
              ...
              startLinkFrom = null;
              console.log("Отменено - кликнули на тот же элемент");
            }
          } else {
      ...
      function createLink(source, target, linkType) {
        ...
        if (linkType === "generalization") {
          ...
          link = new joint.shapes.standard.Link({
            ...
            labels: [
              {
                attrs: { text: { text: "◁ Generalization" } },
              },
            ],
          });
      ...
      function showDiagramElementDetails(element) {
        ...
        let html = `
          <h4 style="margin-top: 0;">Класс: ${classData.name}</h4>
          <p><strong>Тип:</strong> ${classData.type || "Class"}</p>
          <p><strong>Абстрактный:</strong> ${
            classData.isAbstract ? "Да" : "Нет"
          }</p>
          <p><strong>Описание:</strong> ${
            classData.description || classData.description_ru || "Нет описания"
          }</p>

          <h5 style="margin-top: 15px; margin-bottom: 5px;">Атрибуты (${
            attrs.length
          }):</h5>
        `;
        ...
        if (attrs.length > 0) {
          ...
          html += '<p class="text-muted">Нет атрибутов</p>';
        }

        html += `<h5 style="margin-top: 15px; margin-bottom: 5px;">Связи (${links.length}):</h5>`;
        ...
        if (links.length > 0) {
          ...
          links.forEach((l) => {
            ...
            }</strong> → ${l.target_class_name} (${l.relation_kind})</li>`;
          });
          html += "</ul>";
        } else {
          html += '<p class="text-muted">Нет связей</p>';
        }
      ...
      function renderEditorModeDetailsPanel() {
        ...
        if (!activeLeftItem) {
          modelObjectPanel.innerHTML = `
            <h4 style="margin-top: 0;">Объект модели</h4>
            <div class="text-muted" style="padding: 10px 0;">Выберите объект в дереве моделей для просмотра деталей</div>
          `;
          return;
        }
        ...
        if (!info || !info.data) {
          modelObjectPanel.innerHTML = `
            <h4 style="margin-top: 0;">Объект модели</h4>
            <div class="text-muted" style="padding: 10px 0;">Нет данных для отображения</div>
          `;
          return;
        }
        ...
        if (!isElement) {
          modelObjectPanel.innerHTML = `
            <h4 style="margin-top: 0;">${info.kind === 'package' ? 'Пакет' : info.kind === 'model' ? 'Модель' : 'Объект'}: ${esc(data.name)}</h4>
            <p style="padding: 10px 0;">${esc(data.description || data.description_ru || 'Нет описания')}</p>
          `;
          return;
        }
        ...
        let html = `
          <div class="tabs">
            <div class="tab active" onclick="switchEditorTab('description')">Описание</div>
            <div class="tab" onclick="switchEditorTab('attributes')">Атрибуты (${attrs.length})</div>
            <div class="tab" onclick="switchEditorTab('links')">Связи (${filteredLinks.length})</div>
          </div>

          <div class="tab-content active" data-tab="description">
            <h4 style="margin-top: 0;">Класс: ${esc(data.name)}</h4>
            <p><strong>Тип:</strong> ${esc(data.type || 'Class')}</p>
            <p><strong>Абстрактный:</strong> ${data.isAbstract ? 'Да' : 'Нет'}</p>
            <p><strong>Описание:</strong> ${esc(data.description || data.description_ru || 'Нет описания')}</p>
        ...
        if (attrs.length > 0) {
          ...
          html += `
              <div class="attributes-table-empty">Нет атрибутов</div>
            </div>
          `;
        }

        html += '</div><div class="tab-content" data-tab="links"><h4 style="margin-top: 0;">Связи</h4>';
        ...
        if (filteredLinks.length > 0) {
          ...
          html += `
              <div class="attributes-table-empty">Нет связей для отображения</div>
            </div>
          `;
        }
        html += '</div>';
      ...
      function renderEditorModeProfilePanel() {
        ...
        if (!activeRightItem) {
          profileObjectPanel.innerHTML = `
            <h4 style="margin-top: 0;">Объект профиля</h4>
            <div class="text-muted" style="padding: 10px 0;">Выберите объект в дереве профиля для просмотра деталей</div>
          `;
          return;
        }
        ...
        if (!isElement) {
          profileObjectPanel.innerHTML = `
            <h4 style="margin-top: 0;">${info.kind === 'package' ? 'Пакет' : 'Объект'}: ${esc(data.name)}</h4>
            <p style="padding: 10px 0;">${esc(data.description || data.description_ru || 'Нет описания')}</p>
          `;
          return;
        }
        ...
        let html = `
          <div class="tabs">
            <div class="tab active" onclick="switchProfileTab('description')">Описание</div>
            <div class="tab" onclick="switchProfileTab('attributes')">Атрибуты (${attrs.length})</div>
            <div class="tab" onclick="switchProfileTab('links')">Связи (${filteredLinks.length})</div>
          </div>
      ...
      function excludeSelectedAttributes() {
        if (selectedProfileAttributes.size === 0) {
          alert('Выберите атрибуты для исключения');
          return;
        }

        // TODO: Implement attribute exclusion logic
        alert(`Выбрано атрибутов для исключения: ${selectedProfileAttributes.size}\n\nФункциональность исключения будет реализована позже`);
      ...
      function excludeSelectedLinks() {
        if (selectedProfileLinks.size === 0) {
          alert('Выберите связи для исключения');
          return;
        }

        // TODO: Implement link exclusion logic
        alert(`Выбрано связей для исключения: ${selectedProfileLinks.size}\n\nФункциональность исключения будет реализована позже`);
      ...
      function transferSelectedLinks() {
        if (selectedLinks.size === 0) {
          alert('Выберите связи для переноса');
          return;
        }

        // TODO: Implement link transfer logic
        alert(`Выбрано связей: ${selectedLinks.size}\n\nФункциональность переноса будет реализована позже`);
      ...
      // выбирает свойства объекта по ключу
        ...
        if (info.kind === "element") {
          ...
          if (attrs.length) {
            ...
            html +=
              ...
              tableWithHead(["Имя", "Тип", "Кратность", "Описание"], rows);
          } else {
            html +=
              sectionTitle("Атрибуты") +
          ...
          if (links.length) {
            ...
            html +=
              sectionTitle("Связи") +
              tableWithHead(
                ["Имя", "Вид связи", "Целевой класс", "Кратность", "Описание"],
                rows
              );
          } else {
            html +=
              sectionTitle("Связи") +
              '<div class="text-muted">Нет связей</div>';
          }
