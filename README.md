# NauCim

# Nautilus.CIM - Интерактивный прототип

## Описание

Кликабельный прототип платформы Nautilus.CIM для создания, редактирования и управления
профилями информационных моделей электроэнергетических систем по стандарту CIM.

Приложение может работать в двух режимах:

- **Local (по умолчанию)**: статический фронтенд, данные загружаются из JSON (демо-набор).
- **Backend (опционально)**: статический фронтенд + локальный Node.js API с персистентностью (SQLite).

## Структура файлов

```
NauCIM/
├── index.html                  # Главная страница
├── projects.html               # Управление проектами
├── profile-editor.html         # Редактор профилей
├── project-details.html        # Редактор проекта
├── compare.html                # Сравнение профилей
├── src/
│   ├── app/
│   │   ├── pages/           #  js модули страницы.  контроллер страницы
│   ├── services/              # Бизнес-логика и операции с данными
│   │   ├── dataloader.js     # ⭐ Загрузка данных в MemoryStore
│   │   ├── model-service.js
│   │   ├── profile-service.js
│   │   └── project-service.js
│   ├── store/
│   │   └── memory-store.js   # ⭐ Хранилище данных в памяти
│   ├── ui/
│   │   ├── components/       # Переиспользуемые компоненты (модалки)
│   │   ├── renderers/        # Модули рендеринга
│   │   └── sidebar/          # ⭐ Компоненты сайдбара
│   └── styles/                # ⭐ Все CSS файлы
├── docs/                       # Документация
│   ├── ARCHITECTURE.md        # Описание архитектуры
│   ├── CONVENTIONS.md         # Соглашения о разработке
│   └── DATA_STRUCTURES.md  # Контракт данных
│   └── BACKEND_LOCAL.md        # Как запустить локальный backend
├── backend/                    # Локальный backend (Node.js + SQLite + Prisma)
├── data.js                     # ⚠️ УСТАРЕЛО - не использовать
├── sidebar.js                  # ⚠️ УСТАРЕЛО - не использовать
├── styles.css                  # ⚠️ УСТАРЕЛО - не использовать
└── README.md                   # Инструкция
```

### ⚠️ Устаревшие файлы (не использовать для новой разработки)

- **`data.js`** - заменен на `src/services/dataloader.js` и `src/services/*-service.js`
- **`sidebar.js`** - заменен на `src/ui/sidebar/*`
- **`styles.css`** - заменен на `src/styles/**`

Эти файлы сохранены только для совместимости. Вся новая разработка должна использовать модульную архитектуру в директории `src/`.

## Как использовать

⚠️ **ВАЖНО**: Приложение требует локальный HTTP-сервер для работы (из-за использования `fetch()` для загрузки JSON).

### Быстрый старт

1. Откройте терминал в папке проекта
2. Запустите локальный сервер:
   ```bash
   python -m http.server 8000
   ```
   Или на некоторых системах:
   ```bash
   python3 -m http.server 8000
   ```
3. Откройте браузер по адресу:
   ```
   http://localhost:8000
   ```

Подробные инструкции см. в [docs/RUN_LOCAL.md](docs/RUN_LOCAL.md)

### (Опционально) Запуск с backend-персистентностью

См. [docs/BACKEND_LOCAL.md](docs/BACKEND_LOCAL.md) (как запустить backend и включить режим backend во фронтенде).

### Страницы приложения

- **Главная** (`index.html`) → `src/app/pages/index-page.js` - Дашборд, последние проекты
- **Проекты** (`projects.html`) → `src/app/pages/projects-page.js` - Управление проектами
- **Редактирование проекта** (`project-details.html`) → `src/app/pages/project-details-page.js` - Модели, профили, классы
- **Сравнение** (`compare.html`) - Сравнение профилей
- **Редактор профилей** (`profile-editor.html`) - Визуальный редактор с диаграммами

## Основные функции

✅ Создание и управление проектами
✅ Импорт и просмотр моделей CIM
✅ Создание профилей на основе моделей
✅ Сравнение профилей
✅ Визуализация структуры классов
✅ Генерация XSD кода
✅ Генерация документации
✅ Древовидная навигация
✅ Поиск и фильтрация

## Примечания

- Приложение загружает демо-данные из JSON-файлов в папке `models-data/`
- UI-состояние сохраняется в localStorage браузера
- Для полноценной работы требуется современный браузер с поддержкой ES6+
- **Требуется локальный HTTP-сервер** - не открывайте `index.html` напрямую в браузере

## Документация

- 📘 **[Структуры данных V5](docs/DATA_STRUCTURES.md)** - Канонический контракт данных (Project, Model, Profile, Class, Attribute)
- 📥 **[Импорт модели из XMI (JSON)](docs/XMI_IMPORT_DATA_STRUCTURES.md)** - Структуры данных и формат JSON для импорта (XMI → JSON Schema 2020-12)
- 🏗️ **[Архитектура](docs/ARCHITECTURE.md)** - Структура приложения и границы модулей
- 📝 **[Соглашения](docs/CONVENTIONS.md)** - Правила кодирования и контракты данных
- 🚀 **[Запуск локально](docs/RUN_LOCAL.md)** - Как запустить приложение на локальном сервере
- 🗄️ **[Локальный backend](docs/BACKEND_LOCAL.md)** - Как запустить API + SQLite и включить режим backend
- 🐍 **[Python-скрипты](docs/PYTHON_SCRIPTS.md)** - Утилиты для работы с JSON-данными

## Технические детали

### Технологии

- **HTML5** - структура страниц
- **CSS3** - стили и анимации
- **JavaScript ES6+** - логика и интерактивность
- **SVG** - отрисовка связей между классами
- **Drag and Drop API** - перетаскивание элементов
- **localStorage** - хранение данных локально

Опционально (режим backend):

- **Node.js** + **Express** - локальный API
- **SQLite** - легковесная БД
- **Prisma** - ORM и миграции

### Совместимость

- Chrome 90+
- Firefox 88+
- Safari 14+
- Edge 90+

### Ограничения

- По умолчанию приложение работает без сервера (демо-данные из JSON)
- Для персистентного хранения проектов используйте локальный backend (см. [docs/BACKEND_LOCAL.md](docs/BACKEND_LOCAL.md))
- Требуется локальный HTTP-сервер для работы (см. [RUN_LOCAL.md](docs/RUN_LOCAL.md))
- При очистке кэша браузера UI-настройки удаляются

Прототип разработан на основе концепции **Nautilus.CIM v0.0.3** для демонстрации функциональности платформы.

## Структуры данных

Все структуры данных (Project, Model, Profile, Class, Attribute, Package) определены в каноническом документе:

📘 **[docs/DATA_STRUCTURES.md](docs/DATA_STRUCTURES.md)** - Единственный источник истины для контрактов данных

Этот документ содержит:

- TypeScript-интерфейсы для всех сущностей
- Описание обязательных и необязательных полей
- Перечисления (Enums) для типов данных
- Примеры JSON для каждой структуры
- Правила валидации

**Важно**: При работе с данными всегда обращайтесь к этому документу.

---

# Nautilus.CIM v2.0 - Интерактивный прототип с редактором диаграмм

## 🎉 Новые возможности

### 📐 Редактор диаграмм классов (diagram.html)

**Полнофункциональный визуальный редактор для создания UML-диаграмм классов:**

- ✅ **Drag-and-drop** - перетаскивайте классы из левой панели на холст
- ✅ **Два типа связей:**
  - ◁ **Наследование** (обобщение) - полая треугольная стрелка
  - → **Ассоциация** - простая стрелка
- ✅ **Интерактивное редактирование:**
  - Перемещение классов с автоматическим обновлением связей
  - 4 точки соединения на каждом классе (сверху, снизу, слева, справа)
  - Удаление классов через кнопку ×
- ✅ **Сеточный холст** для точного позиционирования
- ✅ **Панорамирование** средней кнопкой мыши
- ✅ **Сохранение** диаграмм в localStorage

## Примеры использования

### Создание иерархии классов

1. Добавьте родительский класс (например, `IdentifiedObject`)
2. Добавьте дочерние классы (например, `PowerSystemResource`, `AssetInfo`)
3. Переключитесь в режим **◁ Наследование**
4. Соедините дочерние классы с родительским
5. Получится классическая UML-иерархия с полыми стрелками

### Создание ассоциаций

1. Добавьте два связанных класса
2. Переключитесь в режим **→ Ассоциация**
3. Соедините классы
4. Простая стрелка покажет направление ассоциации

---

## 📚 Документация для разработчиков

Для разработчиков и контрибьюторов доступна подробная документация:

- **[ARCHITECTURE.md](docs/ARCHITECTURE.md)** - Описание архитектуры приложения
- **[CONVENTIONS.md](docs/CONVENTIONS.md)** - Соглашения о разработке и стиль кода
- **[DATA_STRUCTURES.md](docs/DATA_STRUCTURES.md)** - Контракт данных (канонический источник)
- **[XMI_IMPORT_DATA_STRUCTURES.md](docs/XMI_IMPORT_DATA_STRUCTURES.md)** - Формат импорта модели из XMI (JSON)
- **[.github/copilot-instructions.md](.github/copilot-instructions.md)** - Руководство для GitHub Copilot

### Модульная архитектура

Приложение использует модульную архитектуру с четким разделением ответственности:

- **Слой данных**: `src/services/dataloader.js` + сервисы + `src/store/memory-store.js`
- **Слой UI**: `src/ui/components/` и `src/ui/renderers/`
- **Слой стилей**: `src/styles/**`

См. [ARCHITECTURE.md](docs/ARCHITECTURE.md) для деталей.
