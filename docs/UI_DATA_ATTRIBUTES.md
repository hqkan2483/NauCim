...
1. **If a feature requires navigation/search in the tree and the UI doesn't have enough context:**
   - First, **extend the DOM contract** by adding the missing canonical `data-*` attributes in the renderer(s).
   - Only after that, implement the feature logic.

2. **No fallback tree traversal:**
   - Do not “infer” parents/context by scanning exported project trees.
   - Missing `data-*` is a contract problem, not a reason to add fallbacks.
